...
function resolveRemoteBaseUrl() {
  return process.env.TEST_BASE_URL || 'https://clickup.ssc.one';
}
...
async function startLocalServer() {
  const port = parseInt(process.env.TEST_SERVER_PORT || '9877', 10);
  ...
  const env = {
    ...
    PORT: String(port),
    HOST: '127.0.0.1',
    CLICKUP_ACCESS_TOKEN: process.env.CLICKUP_ACCESS_TOKEN || 'test-token',
